...
- Arm C-self on two pilot questions: one right, one wrong. The wrong one (the Kashmir cardiologist) shipped as supported with confidence 1.0; the judge fell back to do-not-know once in 120 requests; 4 to 8 minutes per question against 5 seconds for arm D. Result file results/C-self_simpleqa_pilot-20260919.json. The full pilot and the 500-question web track run unattended through `scripts/run_web_track.py`, which finishes the freeze, re-searches short questions when a probe shows more than one engine answering, runs A, B and D with `--resume`, and grades. At its start only DuckDuckGo and Startpage answered.
